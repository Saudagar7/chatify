...
import { ShieldIcon, UsersIcon, SearchIcon, XIcon, CheckIcon } from "lucide-react";
...
function SettingsModal({ isOpen, onClose }) {
	...
	return (
		...
		>
			<div
				className="w-full max-w-3xl rounded-2xl border border-slate-800/80 bg-slate-900 shadow-2xl"
				onClick={(e) => e.stopPropagation()}
			>
				<div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
					<div>
						<p className="text-xs uppercase tracking-wide text-slate-500">Privacy</p>
						<h2 className="text-xl font-semibold text-white">Profile photo visibility</h2>
					</div>
					<button
						type="button"
